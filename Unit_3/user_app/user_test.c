...
void read_data_chardev() {
    ...
    if(ret < 0)
        printf("Count not read a message from %s\n", DEVICE_NODE);
    else
...
void write_data_chardev() {
    ...
    fgets(user_buf, sizeof(user_buf) + 1, stdin);
    // scanf("%[^\n]s", user_buf);
    ...
    close_chardev(fd);
    if(ret < 0)
        printf("Could not write message to %s\n", DEVICE_NODE);
    else
...
void control_write_chardev() {
    ...
    ioctl(fd, CTRL_WRITE_CHARDEV, (status_t *)&status);
    close_chardev(fd);
